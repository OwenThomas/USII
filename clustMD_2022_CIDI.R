...
#max number of depressed episodes registered
n_max_der <- 7

use_depression[which(use_depression$n_29033_0_0>(n_max_der-1)),]$n_29033_0_0 <- n_max_der

use_depression[which(use_depression$n_29033_0_0==-4),]$n_29033_0_0 <- n_max_der
...
#29017 Time of day is not really ordinal.
...
#for(G_here in G_seq){
#  m.clustMD.men.2022.G_here <- clustMD(as.matrix(ud_men), G = G_here, CnsInd = 2, OrdIndx = 26, Nnorms = 20000, MaxIter=500, model = "VVI")
#  saveRDS(m.clustMD.men.2022.G_here, file = paste("./2022_fitted_models_v3/m.clustMD.men.2022.",G_here,".rds",sep=""))
#  rm(m.clustMD.men.2022.G_here)
#  gc()
#}

#for(G_here in G_seq){
#  m.clustMD.women.2022.G_here <- clustMD(as.matrix(ud_women), G = G_here, CnsInd = 2, OrdIndx = 26, Nnorms = 20000, MaxIter=500, model = "VVI")
#  saveRDS(m.clustMD.women.2022.G_here, file = paste("./2022_fitted_models_v3/m.clustMD.women.2022.",G_here,".rds",sep=""))
#  rm(m.clustMD.women.2022.G_here)
#  gc()
#}
